...
## Final queries v.0.1 (without custom)
...
- **Q4.1** Latest time in environment ($A$, $l$): Return the latest time for which agents $a$ &in; $A$ were in an environment $l$.

- **Q4.2** Latest measurement in environment($A$, $l$): Return the latest measurement for each agent that performed measurements in a given environment $l$.

**Q5** InverseCoverage($L$, &tau;) lists all agents that can generate measurements of a given type &tau; in the polygon specified in $L$.

**Q6** Observations (&tau; , $cond$, $t_b$, $t_e$): selects measurements generated by agents of given type &tau; in the time range [$t_b$; $t_e$] that satisfy the condition $cond$.

**Q7** Trajectories($t_b$, $t_e$, $l_b$, $l_e$): retrieves the names of agents who went from location $l_b$ to location $l_e$ during the time interval [$t_b$; $t_e$]

**Q8** CoLocate($a$ &in; A, $t_b$, $t_e$): retrieves all agents who were in the same environment as agent $a$ during the specifoed time period.
